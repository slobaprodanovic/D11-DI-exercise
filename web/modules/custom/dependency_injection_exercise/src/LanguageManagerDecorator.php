...
/**
 * Decorator for the LanguageManager service.
 */
class LanguageManagerDecorator implements LanguageManagerInterface {
  ...
  /**
   * Flag to ensure logging happens only once per request.
   *
   * @var bool
   */
  ...
  public function getCurrentLanguage($type = LanguageInterface::TYPE_INTERFACE): LanguageInterface {
    ...
    if ($route === 'dependency_injection_exercise.rest_output_controller_photos' && !$this->logged) {
   ...
   * {@inheritDoc}
   */
  public function reset($type = NULL): LanguageManagerInterface {
   ...
   * {@inheritDoc}
   */
  public function setConfigOverrideLanguage(?LanguageInterface $language = NULL): LanguageManagerInterface {
